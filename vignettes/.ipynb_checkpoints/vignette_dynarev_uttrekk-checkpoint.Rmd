...
Funksjonen finnes i pakken [fellesr](https://github.com/statisticsnorway/fellesr) og fungerer i RStudio (Windows og Linux) og Jupyterlab i produksjonssonen.
...
dynarev <- dynarev_uttrekk(delregnr = num,               # Må oppgis
                            ...
                            enhets_type = "char",        # Default = c("BEDR", "FRTK)
...
**enhets_type** : Velg enhetstype fra Dynarev (f.eks. BEDR og/eller FRTK) <br />
**skjema_cols** : Velg kolonner fra skjemadata. Hvis TRUE henter man alle variabler. Hvis FALSE henter man ikke skjemadata, kun SFU-data (hvis sfu_cols = TRUE). Det er også mulig å lage en vektor/liste av variabelnavn, f.eks. c("variabel1", "variabel2"), dersom man kun vil ha utvalgte skjemakolonner.  <br />
**sfu_cols** : Hvis TRUE blir alle variabler fra SFU for valgt delregister og skjema inkludert. For å kun velge én eller flere variabler fra SFU skrives disse i en vektor, f.eks. c("variabel1", "variabel2"). <br />
